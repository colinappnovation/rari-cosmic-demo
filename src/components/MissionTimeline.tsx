        ...
        {sorted.map(mission => (
          <div key={mission.id} className="relative flex gap-6">
            <div className="relative z-10 flex-shrink-0">
              <div
                className={`w-12 h-12 rounded-full border-4 border-surface/50 shadow-sm flex items-center justify-center text-xs font-bold text-white ${statusDot[mission.status]}`}
              >
            ...
            <div className="flex-1 glass-panel rounded-xl p-5">
              <div className="flex items-start justify-between mb-2">
                ...
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusStyles[mission.status]}`}
